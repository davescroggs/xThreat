...
get_match_chains <- function(season = year(Sys.Date()), round = NA) {
  ...
  players <- fitzRoy::fetch_player_details_afl(season) %>% 
    select(playerId = providerId, firstName, surname, team, position)
  # players <- get_players()
  chains <- inner_join(chains, games, by = "matchId")
  chains <- left_join(chains, players, by = "playerId")
  chains <- chains %>% 
    select(season,roundNumber,homeTeam.teamName:date,venue.name,
           venueWidth:homeTeamDirectionQtr1,displayOrder,chain_number,
           initialState,finalState,period,periodSeconds,team,playerId,
           firstName,surname,position,description,disposal:y)
...
#     players <- players[[5]]
#     players <- players %>% select(playerId,playerName.givenName,playerName.surname)
#     
#     return(players)
# }
